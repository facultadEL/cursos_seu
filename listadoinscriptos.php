          ...
          <td width="75%"><select name="cursoF" size="1" class="myTextField" id="cursoF" onChange="evaluaring()" onkeyup=fn(this.form,this) >
            ...
            <?php
                                         ...
                                         include_once "conexionCursosExtension.php";										 		
                                                 ...
                                                 $tip1 = pg_query($conn,"SELECT id_cursos,nombre,duracion_desde,duracion_hasta FROM cursos where anio=".$_REQUEST["anio"].";");
                                                  while($row1 = pg_fetch_array($tip1)){
...
$tip29 = pg_query($conn,"select inscriptosxcurso.id_inscriptosxcurso,cursos.nombre as nombrecurso,inscripto.id_inscripto, inscripto.localidad, inscripto.nombre as nombre,apellido,dni ,direccion, localidad, mail, telfijo, telcel from inscripto full outer join inscriptosxcurso on (inscriptosxcurso.fk_inscriptos=inscripto.id_inscripto) full outer join cursos on (inscriptosxcurso.fk_curso=cursos.id_cursos) where cursos.id_cursos=$cursoF order by inscripto.apellido,inscripto.nombre ASC ");
 if($cursoF!='' or $cursoF!=0 ){?>
    ...
    <table width="100%" border="1">
      <tr>
        <td width="36%" bgcolor="#666666"><strong>Apellido</strong></td>
        <td width="43%" bgcolor="#666666"><strong>Nombre</strong></td>
		<td width="43%" bgcolor="#666666"><strong>Tel Fijo</strong></td>
		<td width="43%" bgcolor="#666666"><strong>Tel Cel</strong></td>
		<td width="43%" bgcolor="#666666"><strong>E - Mail</strong></td>
        <td width="10%" bgcolor="#666666"><strong>Modificar</strong></td>
        <td width="11%" bgcolor="#666666"><strong>Cup&oacute;n Cuotas</strong></td>
        </tr>
       <? $var=0;
      ...
      <tr>
        ...
        <td align="center" <?=$color?>><a href="modinscripto.php?id=<?= $row29["id_inscripto"] ?>&id_curso=<?= $cursoF ?>"><img src="images/nuevo.png" alt="" width="20" height="20" /></a></td>
        <td align="center" <?=$color?>><a href="imprimircertinscripto.php?inscriptosxcurso=<?= $row29["id_inscriptosxcurso"] ?>"><img src="images/impresora.jpg" alt="" width="20" height="20" align="middle" /></a></td>
        </tr>
        <? }?>
    </table>
    <? }?>
