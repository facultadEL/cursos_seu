...
<body>
<form action="guardarEstadoAnio.php" method="post" name="estadoCursosAnio">
<?php
include_once 'conexionCursosExtension.php'; 
$anio = date('Y');
$val = pg_query("SELECT anio,anio_activado FROM cursos GROUP BY anio,anio_activado ORDER BY anio");
echo '<table align="center" cellspacing="0" cellpadding="2" border="0" width="100%" bgcolor=#585858 id="tabla">';
	echo '<tr bgcolor="#FFFFFF" width="100%">';
		echo '<td id="titulo3" colspan="4" align="center"><l3>Cursos</l3></td>';
	echo '</tr>';
	echo '<tr bgcolor="#000000" width="100%">';
		echo '<td align="center" width="50%"><strong><label1>A&ntilde;o</label1></strong></td>';
...
while($row=pg_fetch_array($val,NULL,PGSQL_ASSOC)){
	echo '<tr>';
		echo '<td align="left" id="renglon"><l2>'.$row['anio'].'</l2></td>';
		if($row['anio_activado']=='t'){
			//$variable = 'Activado';
		...
		if($row['anio_activado']=='t'){
			echo '<td align="center" id="registro">';
					echo '<input type="checkbox" name="ckAnio'.$row['anio'].'" checked/>';
			echo '</td>';
		}else{
			echo '<td align="center" id="registro">';
					echo '<input type="checkbox" name="ckAnio'.$row['anio'].'"/>';
			echo '</td>';
		}
	echo '<tr>';
}
