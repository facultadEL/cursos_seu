...
$sql= pg_query($conn,"select cursos.cantcuota, cursos.monto, cursos.monto_antes_venc, cursos.dia_venc, inscriptosxcurso.porcdescuento,inscripto.id_inscripto, inscripto.nombre as nombre, apellido, cursos.nombre as curnom, cursos.duracion_desde,id_inscriptosxcurso from inscriptosxcurso full outer join inscripto on(inscriptosxcurso.fk_inscriptos = inscripto.id_inscripto) full outer join cursos on(inscriptosxcurso.fk_curso = cursos.id_cursos) where inscriptosxcurso.id_inscriptosxcurso=$inscripto");
...
for ($contarcuota=0; $contarcuota<$row29["cantcuota"]; $contarcuota++){
  ...
  if(($mes[1]+$contarcuota)>12)
  {
    $vencmes=($mes[1]+$contarcuota) - 12;
    $anio = date("Y") + 1;
    $anioCorto = date("y") + 1;
  }
  else
      ...
      <tr>
        <td width="50%"><table width="100%" border="0" cellspacing="0" cellpadding="0">
          ...
          <tr>
            ...
            <td colspan="2" rowspan="3" align="center"><img src="generarcodbarra/barcode.php?code=<? echo $inscripto.$contcuota.$row29["cantcuota"].$montomostrar.$anioCorto;?>&amp;scale=1" alt="" /></td>
            </tr>
            <?php
         	if($contarcuota != 0)
         	{
          ...
          <tr>
        	...
        	{
        	?>
        	<td><strong><span class="fuente">Monto:</span></strong></td>
        	<td width="42%"><span class="fuente">
              <?=$montomostrardesc2?>
            </span></td>
        	<?php	
